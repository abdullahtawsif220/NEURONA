...
def init_db():
    if not os.path.exists(DB_NAME):
        ...
        try:
            ...
            pass  # Column already exists
        try:
            c.execute("ALTER TABLE ideas ADD COLUMN product_image TEXT")  # ✅ Add column for existing DBs
        except sqlite3.OperationalError:
            pass
        conn.commit()
        conn.close()
...
def submit_idea():
    ...
    if request.method == 'POST':
        ...
        if creator:
            ...
            ''', (creator_id, title, category, tags, stage, industry, summary, description, funding, equity, contact_email, product_image_filename))
            conn.commit()
            conn.close()
